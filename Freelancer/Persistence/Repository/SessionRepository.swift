...
class SessionRepository: Repository<Session> {
    ...
    func saveSession(_ sessionDTO: SessiontDTO) {
        do {
            try super.save(object: sessionDTO.mapToPersistenceObject())
    ...
    func getAllSessions(on sort: Sorted? = nil ) -> [SessiontDTO] {
        return super.fetch(
            ...
            predicate: nil,
            sorted: sort
        ).map { SessiontDTO.mapFromPersistenceObject($0 as! Session) }
    }
    ...
    func delete(_ sessiontDTO: SessiontDTO, predicate: NSPredicate) {
        do {
            try super.delete(
                Session.self,
                object: sessiontDTO.mapToPersistenceObject(),
                predicate: predicate
            )
        } catch { print(error.localizedDescription) }
    }
    ...
    func deleteAll() {
        do {
            try super.deleteAll(Session.self)
        } catch { print(error.localizedDescription) }
    }

    func update(_ sessiontDTO: SessiontDTO) {
        do {
            try super.update(object: sessiontDTO.mapToPersistenceObject())
        } catch { print(error.localizedDescription) }
    }
}
